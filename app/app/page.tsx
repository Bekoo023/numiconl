...
const catColors = ["#3b82f6", "#a855f7", "#f97316", "#14b8a6", "#ec4899"];

const categories = [
  { name: "Boodschappen", pct: 34, value: eur(1104) },
  { name: "Reiskosten", pct: 22, value: eur(715) },
  { name: "Software", pct: 18, value: eur(585) },
  { name: "Kantoor", pct: 15, value: eur(487) },
  { name: "Horeca", pct: 11, value: eur(357) },
];

const bookings = [
  { date: "14 jun", vendor: "Albert Heijn", cat: "Boodschappen", vat: eur(2.31), amount: eur(27.45), status: "Geboekt" },
  { date: "13 jun", vendor: "NS Reizigers", cat: "Reiskosten", vat: eur(0), amount: eur(18.6), status: "Geboekt" },
  { date: "12 jun", vendor: "Adobe", cat: "Software", vat: eur(12.18), amount: eur(70.0), status: "Geboekt" },
  { date: "11 jun", vendor: "Coffee District", cat: "Horeca", vat: eur(0.79), amount: eur(9.5), status: "In behandeling" },
  { date: "10 jun", vendor: "Coolblue", cat: "Kantoor", vat: eur(15.61), amount: eur(89.95), status: "Geboekt" },
  { date: "09 jun", vendor: "Shell", cat: "Reiskosten", vat: eur(11.55), amount: eur(66.4), status: "In behandeling" },
];
...
// Volgende Nederlandse btw-aangifte (kwartaal): 31 jan, 30 apr, 31 jul, 31 okt.
...
export default async function AppPage() {
  ...
  const totalSpend = monthly.reduce((a, d) => a + d.v, 0);

  const vat = nextVatDeadline(new Date());
      ...
      <aside className="hidden lg:flex flex-col w-60 fixed inset-y-0 border-r border-white/[0.06] bg-[#0d0f1e]">
          ...
          {navItems.map((item) => (
            ...
            >
              <div className="flex items-center gap-3">
                <svg
                  className={`flex-shrink-0 ${item.active ? "text-accent" : "text-slate-500 group-hover:text-slate-300"}`}
        ...
        <div className="mx-3 mb-3 p-3 rounded-xl bg-amber-500/8 border border-amber-500/15">
          <div className="flex items-start gap-2">
            ...
            <div>
              <p className="text-xs text-amber-400 font-medium leading-snug">3 bonnetjes wachten</p>
              <p className="text-[10px] text-slate-500 mt-0.5">Klik om te verwerken</p>
            </div>
          </div>
        </div>
        ...
        <div className="px-3 py-4 border-t border-white/[0.06]">
          <div className="flex items-center gap-2.5 px-2 mb-3">
            <div
              ...
              style={{ backgroundColor: `${avatarColor(firstName)}20`, color: avatarColor(firstName) }}
            >
              {initials}
            </div>
        ...
        <main className="px-5 py-7 lg:px-9 lg:py-9 max-w-[1080px]">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            <div>
              ...
              <h1 className="display text-white text-3xl sm:text-4xl leading-tight">
                {greeting}, {firstName}.
              </h1>
            </div>
            <UploadReceiptButton />
          ...
          <div className="flex items-center gap-4 bg-white/[0.03] border border-white/[0.07] rounded-2xl px-5 py-4 mb-5">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm text-white">
                Volgende btw-aangifte <span className="text-slate-500">· {vatLabel}</span>
              </p>
              <p className="text-xs text-slate-500 mt-0.5">
                Zorg dat al je bonnetjes op tijd verwerkt zijn.
              </p>
            </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
            {stats.map((s) => (
              <div
                key={s.label}
                className="bg-white/[0.03] border border-white/[0.07] rounded-2xl p-4 lg:p-5 hover:border-white/[0.12] transition-colors"
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="w-9 h-9 rounded-xl bg-accent/10 flex items-center justify-center flex-shrink-0">
                    <svg className="text-accent" style={{ width: "18px", height: "18px" }} fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24" aria-hidden="true">
                      {s.icon}
                    </svg>
                  </div>
                  <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-md ${
                    s.up ? "bg-accent/10 text-accent" : "bg-rose-500/10 text-rose-400"
                  }`}>
                    {s.delta}
                  </span>
                </div>
            ...
            <div className="bg-white/[0.03] border border-white/[0.07] rounded-2xl p-5 lg:col-span-3">
              <div className="flex items-start justify-between mb-1">
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">
                    Jaar totaal:{" "}
                    <span className="num text-slate-300">{eur(totalSpend)}</span>
                  </p>
                </div>
                <span className="text-[11px] text-slate-500 bg-white/[0.04] px-2.5 py-1 rounded-lg border border-white/[0.06]">
                  2026
                </span>
              </div>
                  ...
                  {monthly.map((d, i) => {
                    ...
                    return (
                      <div key={d.m} className="flex-1 flex flex-col items-center gap-2 h-full justify-end">
                        {isLast && (
                          <span className="num text-[10px] text-accent font-semibold">{eur(d.v)}</span>
                        )}
                        <div className="w-full flex items-end" style={{ height: "100%" }}>
                          <div
                            ...
                            style={{
                              height: `${heightPct}%`,
                              background: isLast
                                ? "linear-gradient(180deg, #46d39a 0%, rgba(70,211,154,0.35) 100%)"
                ...
                <div className="flex gap-2 mt-2">
                  {monthly.map((d) => (
                    <div key={d.m} className="flex-1 text-center">
                      <span className="num text-[11px] text-slate-500">{d.m}</span>
                    </div>
                  ))}
                </div>
              </div>
                  ...
                  <div key={c.name}>
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <div className="flex items-center gap-2.5">
                        <span className="num text-xs text-slate-500">{c.value}</span>
                        <span
                          className="text-[10px] font-semibold px-1.5 py-0.5 rounded-md"
                          style={{ backgroundColor: `${catColors[i]}18`, color: catColors[i] }}
                        >
                          {c.pct}%
                        </span>
                      </div>
                    ...
                    <div className="h-1 rounded-full bg-white/[0.06] overflow-hidden">
                      <div
                        ...
                        style={{ width: `${c.pct}%`, backgroundColor: catColors[i] }}
                      />
                    </div>
                  </div>
                ))}
          ...
          {/* Recente transacties (doorzoekbaar) */}
          <TransactionsTable bookings={bookings} />
        </main>
      </div>
    </div>
  );
